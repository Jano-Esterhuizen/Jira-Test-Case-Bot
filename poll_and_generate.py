...
REQUIRE_PR_LINK = os.environ.get("REQUIRE_PR_LINK", "true").lower() == "true"
MARKER = "🤖 AI-generated test cases"
...
def process_ticket(issue: dict) -> None:
    ...
    if any(MARKER in c for c in comments):
        print(f"  - {key}: already has test cases, skipping")
        return
    ...
    header = f"{MARKER} (review before use)\n"
    if pr:
        header += f"Based on {key} and PR {pr.get('html_url', '')}\n"
    else:
        header += f"Based on {key} only — no accessible PR found.\n"
    header += "-" * 40 + "\n\n"
