...
ticket for the "Test Strategy" comment.
...
## Behavior notes
- The trigger is assignment: as soon as a ticket is assigned to you, the
  next polling cycle picks it up.
- Every generated comment starts with the key phrase "Test Strategy".
- Duplicate protection: if any comment on the ticket already contains
  "Test Strategy", the bot skips it — reruns never double-post.
- No PR link in the comments? The bot generates test cases from the
  description/AC alone, marked as ticket-only inside the comment. (Set
  REQUIRE_PR_LINK=true in .env if you'd rather it wait for the link.)
- Laptop was off? The next run catches up on everything missed.
- To regenerate for a ticket (e.g. the PR link arrived after a ticket-only
  strategy was posted, or the PR changed), delete the bot's "Test Strategy"
  comment and the next run will redo it with the latest information.
